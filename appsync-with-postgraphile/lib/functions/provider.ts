...
export const handler = async (event: any): Promise<any> => {
  ...
  try {
    ...
    const schema = await loadGraphqlSchema(pgPool, PG_SCHEMAS, {
      writeCache: file,
    });
...
const updateLayer = async (file: string) => {
  try {
    ...
    const result = await lambdaClient
      .publishLayerVersion({ LayerName, Content: { ZipFile: zip.toBuffer() } })
      .promise();
    console.log("publishLayerVersion");
    console.log(result);

    const getConfigResult = await lambdaClient
    ...
    const updateLambdaResult = await lambdaClient
      .updateFunctionConfiguration({ FunctionName, Layers })
      .promise();
    ...
    if (removedLayer) {
      ...
      const deleteLayerResult = await lambdaClient
  ...
  try {
    ...
    if (mutationType && isObjectType(mutationType)) {
      ...
      for (const fieldName in fields) {
        ...
        if (op.length > 50) {
          console.log(`skip subscription for ${fieldName}`);
          continue;
        }
    ...
    for (const wrapper of Object.keys(wrappers)) {
      definition = definition.replace(
        new RegExp(`(:\\s+)${wrapper}`, "g"),
        `$1${wrappers[wrapper].fieldType}`
      );
    }

    // remove unused payloads
    definition = definition.replace(
      /(^\s*#.*$)+\s*type\s+\w+Payload\s*{([^}]*)}/gm,
      ""
    );

    // start schema creation
    await appsyncClient.startSchemaCreation({ apiId, definition }).promise();
    let response = await appsyncClient
      .getSchemaCreationStatus({ apiId })
      .promise();
    ...
    while (status !== "SUCCESS") {
      ...
      // status = (
      //   await appsyncClient.getSchemaCreationStatus({ apiId }).promise()
      // ).status;

      response = await appsyncClient
        .getSchemaCreationStatus({ apiId })
        .promise();
    ...
    for (const typeName of typeNames) {
      ...
      for (const fieldName in type.getFields()) {
        ...
        await createOrUpdatePipelineResolver(
          apiId,
          typeName,
          fieldName,
          appsyncFnId,
...
const createOrUpdatePipelineResolver = async (
  ...
  const config: AWS.AppSync.UpdateResolverRequest = {
    ...
    requestMappingTemplate: wrapper
      ? [
          `$util.qr($ctx.stash.put("wrapper", "${wrapper.fieldName}"))`,
          "{}",
        ].join("\n")
      : "{}",
  ...
  try {
    ...
    const result = await appsyncClient
...
const createOrUpdateSubscriptionResolver = async (
  apiId: string,
  fieldName: string
) => {
